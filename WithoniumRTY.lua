...
if debugX then
	warn('Initialising WithoniumRTY')
end
...
local WithoniumRTYFolder = "WithoniumRTY"
local ConfigurationFolder = WithoniumRTYFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		WithoniumRTYOpen = {Type = 'bind', Value = 'Insert', Name = 'WithoniumRTY Keybind'},
	},
	System = {
...
local function callSafely(func, ...)
	if func then
		...
		if not success then
			warn("WithoniumRTY | Function failed with error: ", result)
			return false
		else
			return result
		end
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if callSafely(isfolder, WithoniumRTYFolder) then
				if callSafely(isfile, WithoniumRTYFolder..'/settings'..ConfigurationExtension) then
					file = callSafely(readfile, WithoniumRTYFolder..'/settings'..ConfigurationExtension)
				end
			end
			if useStudio then
				file = [[
		{"General":{"WithoniumRTYOpen":{"Value":"K","Type":"bind","Name":"WithoniumRTY Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"WithoniumRTY Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
			...
			if #file > 0 then
				...
				for settingName, settingValue in overriddenSettings do
					...
					assert(#split == 2, "WithoniumRTY | Invalid overridden setting name: " .. settingName)
	...
	if not success then 
		if writefile then
			warn('WithoniumRTY had an issue accessing configuration saving capability.')
		end
	end
end
...
if false and promptUser == 1 and prompt and type(prompt.create) == "function" then
	prompt.create(
...
local WithoniumRTY = useStudio and script.Parent:FindFirstChild('WithoniumRTY') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local WithoniumRTYDestroyed = false

repeat
	if WithoniumRTY:FindFirstChild('Build') and WithoniumRTY.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('WithoniumRTY | Build Mismatch')
		print('WithoniumRTY may encounter issues as you are running an incompatible interface version ('.. ((WithoniumRTY:FindFirstChild('Build') and WithoniumRTY.Build.Value) or 'No Build') ..').\n\nThis version of WithoniumRTY is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, WithoniumRTY = WithoniumRTY, useStudio and script.Parent:FindFirstChild('WithoniumRTY') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

WithoniumRTY.Enabled = false

if gethui then
	WithoniumRTY.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(WithoniumRTY)
	WithoniumRTY.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	WithoniumRTY.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == WithoniumRTY.Name and Interface ~= WithoniumRTY then
			Interface.Enabled = false
			Interface.Name = "WithoniumRTY-Old"
		end
	end
elseif not useStudio then
...
if WithoniumRTY.AbsoluteSize.X < minSize.X and WithoniumRTY.AbsoluteSize.Y < minSize.Y then
...
local Main = WithoniumRTY.Main
local MPrompt = WithoniumRTY:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = WithoniumRTY:FindFirstChild('Drag')
...
local Notifications = WithoniumRTY.Notifications
local keybindConnections = {}

local SelectedTheme = WithoniumRTYLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = WithoniumRTYLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	WithoniumRTY.Main.BackgroundColor3 = SelectedTheme.Background
	WithoniumRTY.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	WithoniumRTY.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	WithoniumRTY.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	WithoniumRTY.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	WithoniumRTY.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	WithoniumRTY.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		WithoniumRTY.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		WithoniumRTY.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(WithoniumRTY:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	if not id then return assetUri end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('WithoniumRTY had an issue decoding the configuration file, please try delete the file and reopen WithoniumRTY.') return end
	for FlagName, Flag in pairs(WithoniumRTYLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("WithoniumRTY | Unable to find '"..FlagName.. "' in the save file.")
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(WithoniumRTYLibrary.Flags) do
...
function WithoniumRTYLibrary:Notify(data)
	task.spawn(function()
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			WithoniumRTYLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			WithoniumRTYLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping " .. tostring(getSetting("General", "WithoniumRTYOpen")) .. ".", Duration = 7, Image = 4400697855})
		end
	end
...
local function Unhide()
	...
	Main.Visible = true
	TweenService:Create(Main, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Size = useMobileSizing and UDim2.new(0, 500, 0, 275) or UDim2.new(0, 850, 0, 475)}):Play()
	TweenService:Create(Main.Topbar, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Size = useMobileSizing and UDim2.new(0, 500, 0, 45) or UDim2.new(0, 850, 0, 45)}):Play()
...
local function saveSettings()
	...
	if success then
		...
		callSafely(writefile, WithoniumRTYFolder..'/settings'..ConfigurationExtension, encoded)
	end
end
...
local function createSettings(window)
	...
	if Elements['WithoniumRTY Settings'] then
...
local function setVisibility(visibility: boolean, notify: boolean?)
	...
	if false and WithoniumRTY:FindFirstChild('Loading') then
		if getgenv and not getgenv().WithoniumRTYCached then
			WithoniumRTY.Enabled = true
			WithoniumRTY.Loading.Visible = true

			task.wait(1.4)
			WithoniumRTY.Loading.Visible = false
		end
	end

	if getgenv then getgenv().WithoniumRTYCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				WithoniumRTYLibrary:Notify({Title = 'Build Mismatch', Content = 'WithoniumRTY may encounter issues as you are running an incompatible interface version ('.. ((WithoniumRTY:FindFirstChild('Build') and WithoniumRTY.Build.Value) or 'No Build') ..').\n\nThis version of WithoniumRTY is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	if Settings.ToggleUIKeybind then
		...
		if type(keybind) == "string" then
			local success = pcall(function() return Enum.KeyCode[keybind] end)
			if not success then
				success = pcall(function() return Enum.UserInputType[keybind] end)
			end
			assert(success, "ToggleUIKeybind must be a valid KeyCode or UserInputType")
			overrideSetting("General", "WithoniumRTYOpen", keybind)
		elseif typeof(keybind) == "EnumItem" then
			assert(keybind.EnumType == Enum.KeyCode, "ToggleUIKeybind must be a KeyCode enum")
			overrideSetting("General", "WithoniumRTYOpen", keybind.Name)
		else
			error("ToggleUIKeybind must be a string or KeyCode enum")
		end
	end

	ensureFolder(WithoniumRTYFolder)
	...
	if Settings.Icon then
		...
		if logoId then
			...
			Topbar.Title.Position = UDim2.new(0, 40, 0.5, 0)
		end
	end

	Main.Size = useMobileSizing and UDim2.new(0, 500, 0, 275) or UDim2.new(0, 850, 0, 475)
	Main.Visible = true
	...
	local TabListPadding = TabList:FindFirstChildOfClass("UIPadding")
	if TabListPadding then TabListPadding:Destroy() end
	...
	if TabList:IsA("ScrollingFrame") then
		...
		TabList.CanvasSize = UDim2.new(1, 0, 0, 0)
	...
	LoadingFrame.Subtitle.Text = ""

	if false then
		LoadingFrame.Version.Text = "WithoniumRTY UI"
	end

	if Topbar:FindFirstChild('Settings') then Topbar.Settings.Visible = false end
	if Topbar:FindFirstChild('Search') then Topbar['Search'].Position = UDim2.new(1, -75, 0.5, 0) end
		...
		task.spawn(function()
			while true do
				...
				WithoniumRTYLibrary:Notify({
					Title = "WithoniumRTY Interface",
					Content = "If u see this message, is mean our client is working correctly and cleaned u cache",
					Duration = 7,
					Image = 4370033185,
				})
			end
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		ensureFolder(WithoniumRTYFolder.."/Discord Invites")

		if callSafely(isfile, WithoniumRTYFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then
				callSafely(writefile, WithoniumRTYFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"WithoniumRTY RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		ensureFolder(WithoniumRTYFolder.."/Key System")
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("WithoniumRTY | "..Key.." Error " ..tostring(Response))
				end
			end
		end
		...
		if callSafely(isfile, WithoniumRTYFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				local savedKeys = callSafely(readfile, WithoniumRTYFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension)
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						callSafely(writefile, WithoniumRTYFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						WithoniumRTYLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function TargetTab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("WithoniumRTY | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function TargetTab:CreateColorPicker(ColorPickerSettings)
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					task.wait(0.1)
					TweenService:Create(ColorPicker, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {Size = UDim2.new(1, -10, 0, 140)}):Play()
					TweenService:Create(Background, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {Size = UDim2.new(0, 173, 0, 86), Position = UDim2.new(1, -10, 0, 50)}):Play()
					TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {Position = UDim2.new(1, -10, 0, 115), Size = UDim2.new(0, 173, 0, 12)}):Play()
					...
					TweenService:Create(ColorPicker.RGB, TweenInfo.new(0.8, Enum.EasingStyle.Exponential), {Position = UDim2.new(0, 5, 0, 45)}):Play()
					TweenService:Create(ColorPicker.HexInput, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Position = UDim2.new(0, 5, 0, 78)}):Play()
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = 0, BackgroundTransparency = 1}):Play()
					...
					TweenService:Create(Background, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {Size = UDim2.new(0, 39, 0, 22), Position = UDim2.new(1, -10, 0.5, 0)}):Play()
					TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {Position = UDim2.new(1, -10, 0, 110), Size = UDim2.new(0, 173, 0, 12)}):Play()
					...
					TweenService:Create(ColorPicker.RGB, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {Position = UDim2.new(0, 5, 0, 70)}):Play()
					TweenService:Create(ColorPicker.HexInput, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Position = UDim2.new(0, 5, 0, 90)}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					WithoniumRTYLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
		...
		function TargetTab:CreateSection(SectionName)
			...
			function SectionValue:Visible(State)
				Section.Visible = State
				if SectionValue.Divider then
					SectionValue.Divider.Visible = State
				end
			end
		...
		function TargetTab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("WithoniumRTY | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					WithoniumRTYLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function TargetTab:CreateDropdown(DropdownSettings)
			...
			local function SetDropdownOptions()
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseEnter:Connect(function()
					...
					DropdownOption.Interact.MouseLeave:Connect(function()
						if not table.find(DropdownSettings.CurrentOption, Option) then
							TweenService:Create(DropdownOption, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.DropdownUnselected}):Play()
						end
					end)
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("WithoniumRTY | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					WithoniumRTYLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function TargetTab:CreateKeybind(KeybindSettings)
			...
			local connection = UserInputService.InputBegan:Connect(function(input, processed)
				if CheckingForKey then
					...
					if NewKey then
						local NewKeyName = NewKey.Name
						Keybind.KeybindFrame.KeybindBox.Text = tostring(NewKeyName)
						KeybindSettings.CurrentKeybind = tostring(NewKeyName)
						...
						if KeybindSettings.CallOnChange then
							KeybindSettings.Callback(tostring(NewKeyName))
						end
					end
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and not processed then
					local keyEnum
					local success = pcall(function() keyEnum = Enum.KeyCode[KeybindSettings.CurrentKeybind] end)
					if not success then
						pcall(function() keyEnum = Enum.UserInputType[KeybindSettings.CurrentKeybind] end)
					end
					...
					if keyEnum and (input.KeyCode == keyEnum or input.UserInputType == keyEnum) then
						...
						if not KeybindSettings.HoldToInteract then
							...
							if not Success then
								...
								print("WithoniumRTY | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
								task.wait(0.5)
								Keybind.Title.Text = KeybindSettings.Name
								TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					WithoniumRTYLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function TargetTab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= WithoniumRTYLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("WithoniumRTY | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						WithoniumRTYLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function TargetTab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("WithoniumRTY | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					WithoniumRTYLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			WithoniumRTYLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			WithoniumRTYLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end

	function Window:Notify(data)
		return WithoniumRTYLibrary:Notify(data)
	end

	function Window:Destroy()
		return WithoniumRTYLibrary:Destroy()
	end
...
function WithoniumRTYLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection
function WithoniumRTYLibrary:Destroy()
	WithoniumRTYDestroyed = true
	hideHotkeyConnection:Disconnect()
	for _, connection in keybindConnections do
		connection:Disconnect()
	end
	WithoniumRTY:Destroy()
end
...
if false and Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['WithoniumRTY Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	local key = getSetting("General", "WithoniumRTYOpen")
	
	local keyEnum
	local success = pcall(function() keyEnum = Enum.KeyCode[key] end)
	if not success then
		pcall(function() keyEnum = Enum.UserInputType[key] end)
	end

	if keyEnum and (input.KeyCode == keyEnum or input.UserInputType == keyEnum) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function WithoniumRTYLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				WithoniumRTYLibrary:Notify({Title = "WithoniumRTY Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			WithoniumRTYLibrary:Notify({Title = "WithoniumRTY Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('WithoniumRTY Configurations Error | '..tostring(result))
			WithoniumRTYLibrary:Notify({Title = "WithoniumRTY Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
